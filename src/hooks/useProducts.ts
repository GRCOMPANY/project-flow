...
export function useProducts() {
  ...
  const fetchProducts = async () => {
    ...
    if (error) {
      ...
      const mappedProducts = (data || []).map((p) => {
        ...
        const marginPercent = costPrice > 0 ? (marginAmount / costPrice) * 100 : retailPrice > 0 ? 100 : 0;
  ...
  const addProduct = async (
    product: Omit<
      Product,
      "id" | "createdAt" | "updatedAt" | "supplier" | "marginAmount" | "marginPercent" | "marginLevel"
    >,
  ) => {
    const { data, error } = await supabase
      .from("products")
      .insert({
        ...
        status: (product.status || "activo").toLowerCase(),
  ...
  const updateProduct = async (
    id: string,
    updates: Partial<
      Omit<Product, "id" | "createdAt" | "updatedAt" | "supplier" | "marginAmount" | "marginPercent" | "marginLevel">
    >,
  ) => {
    ...
    if (error) {
      toast({
        title: "Error",
        description:
          error.code === "23505" ? "El SKU ya existe, usa otro diferente" : "No se pudo actualizar el producto",
        variant: "destructive",
      });
      return false;
    }
